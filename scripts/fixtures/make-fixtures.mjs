...
// ─────────────────────────────────────────────────────────────────────────
// 4-2. 같은 문장이 여러 번 나오는 문서
//
// 형광펜이 "글자를 다시 찾아서" 칠하는 방식이면 첫 번째 것만 칠하거나 엉뚱한
// 자리를 칠한다. 저장해 둔 위치를 쓰는지 가리는 시험용이다.
// ─────────────────────────────────────────────────────────────────────────

{
  const same = '지원금은 방과후학교 수강료로만 사용할 수 있습니다.'
  // 되풀이되는 문장 사이를 넉넉히 벌려, 각각이 **다른 청크**에 들어가게 한다.
  // 그래야 "두 번째 것을 골랐을 때 두 번째 자리가 칠해지는가" 를 눈으로 볼 수 있다.
  const filler = (n) =>
    `<p>학교장은 대상자를 확정하여 통보한다. 통보를 받은 학부모는 정해진 기간 안에
      신청서를 제출하여야 하며, 기간을 넘긴 경우에는 다음 차수에 신청할 수 있다.
      담당자는 신청 현황을 주 단위로 정리하여 관리한다. (${n}번째 안내)</p>
     <p>집행 잔액이 발생한 경우에는 학기 종료 후 정산하여 반납한다. 정산 결과는
      학교운영위원회에 보고하며, 보고 자료는 5년간 보관한다. 세부 절차는 별도의
      지침으로 정한다. (${n}번째 정산 안내)</p>`
  ...
  htmlToPdf('repeat-ko', doc('되풀이되는 문장', pages.map(page).join('')))
  expect('repeat-ko.pdf', 1, marker(1, 'R1AA'))
  expect('repeat-ko.pdf', 2, marker(2, 'R2BB'))
}
